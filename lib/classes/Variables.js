...
class Variables {
	...
	disableDepedentServices(func) {
		const dependencyMessage = (configValue, serviceName) =>
			`Variable dependency failure: variable '${ configValue }' references ${ serviceName } but using that service requires a concrete value to be called. Some values (including region and stage) are used in the retrival of variables and therefore must be populated before variables can be retrieved.`;
